...
const Home = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await axios.get('https://blog-webapp-2rak.onrender.comapi/posts/');
  ...
  const handleLike = async (postId) => {
    try {
      if (likedPosts.has(postId)) {
        ...
        await axios.delete(`https://blog-webapp-2rak.onrender.comapi/posts/${postId}/like`, {
        ...
        await axios.post(`https://blog-webapp-2rak.onrender.comapi/posts/${postId}/like`, {}, {
          headers: { 'Authorization': `Bearer ${JSON.parse(localStorage.getItem('token'))}` }
        });
  ...
  const handleCommentSubmit = async (postId) => {
    try {
      const response = await axios.post(`https://blog-webapp-2rak.onrender.comapi/posts/${postId}/comment`, {
        content: newComment,
        parent: null
      }, {
        headers: { 'Authorization': `Bearer ${JSON.parse(localStorage.getItem('token'))}` }
